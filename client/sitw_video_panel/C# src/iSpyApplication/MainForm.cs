...
namespace iSpyApplication
{
    ...
    public partial class MainForm : Form
    {
        ...
        //*YL*----------------------------
        private static string str_YL_Version = "2.8";
        private Timer ylCommTimer;
        private static int iStateLayout = 2;
        private static string strNodeName = "NAE";
        private static string strActiveNode = "";

        private int iScreenWidth = Screen.PrimaryScreen.Bounds.Width;
        private int iScreenHeight = Screen.PrimaryScreen.Bounds.Height;
        private static int iWw = 190; //180;
        private static int iWh = 338; //270;
        //private static double iAspectRatio = 1.5;
        private static int iBGA = 8;

        private List<string> strListOnscreenMsg = new List<string>(new string[] {"Smile", "Dance", "Wave"});
        private string strOnscreenMsg = "";
        private int iOnscreenMsgX = 0;
        private int iOnscreenMsgY = 0;
        private int iOnscreenMsgCt = 0;
        private int iOnscreenMsgStyle = 0;
        private int iOnscreenMsgXx = 0;
        private int iOnscreenMsgYy = 0;              

        //private ylExt.ylUnionComm _yluc;
        //private ylExt.ylSocketClient _ylsc;
        //*YL*----------------------------
        ...
        private void MainFormLoad(object sender, EventArgs e)
        {
            ...
            //*YL* ---------------------------------------------------------
            Console.WriteLine("-------YL-------" + str_YL_Version + "-------YL-------");
            
            //_yluc = new ylExt.ylUnionComm(this, "iSpy");
            //_yluc.ylUnionConnect("1234");
            
            //_ylsc = new ylExt.ylSocketClient();
            //_ylsc.ylSocketConnct("localhost", 12345);

            ylCommTimer = new Timer(1000);
            ylCommTimer.Elapsed += ylCommTimerElapsed;
            ylCommTimer.AutoReset = true;
            ylCommTimer.SynchronizingObject = this;
            GC.KeepAlive(ylCommTimer);
            ylCommTimerElapsed(null, null);
            //*YL* ---------------------------------------------------------
        ...
        //*YL* -----------------------------------------------------
        private void ylCommTimerElapsed(object sender, ElapsedEventArgs e)
        {
            ...
            //string text = System.IO.File.ReadAllText(@"d:\txt.txt");
            //Console.WriteLine("xxxxxxxxxx "+ text);
            
            string filepath = @"c:\SitwMsg\sche_ispy.txt";
            string[] lines = {};

            bool bFileExists = System.IO.File.Exists(filepath);
            //Console.WriteLine("----->>> " + bFileExists.ToString());
            if (bFileExists == true)
            {
            ...
            foreach (string line in lines)
            {
                ...
                if (string.Compare(line, "0", true) == 0)
                {
                    iStateLayout = 0;
                }
                else if (string.Compare(line, "1", true) == 0)
                {
                    iStateLayout = 1;                   
                }
                else if (string.Compare(line, "2", true) == 0)
                {
                    iStateLayout = 2;
                }
                else if (string.Compare(line, "3", true) == 0)
                {
                    iStateLayout = 3;
                }
                else if (string.Compare(line, "4", true) == 0)
                {
                    iStateLayout = 4;
                }
                else if (string.Compare(line, "NAE", true) == 0)
                {
                    strNodeName = "NAE";
                }
                else if (string.Compare(line, "Broadway", true) == 0)
                {
                    strNodeName = "Broadway";
                }
                else if (string.Compare(line, "Leytonstone", true) == 0)
                {
                    strNodeName = "Leytonstone";
                }
                else if (string.Compare(line, "Mill", true) == 0)
                {
                    strNodeName = "Mill";
                }
                else if (string.Compare(line, "le", true) == 0)
                {
                    strActiveNode = "Leytonstone";
                }
                else if (string.Compare(line, "bw", true) == 0)
                {
                    strActiveNode = "Broadway";
                }
                else if (string.Compare(line, "wa", true) == 0)
                {
                    strActiveNode = "Mill";
                }
                else if (string.Compare(line, "na", true) == 0)
                {
                    strActiveNode = "NAE";
                }                
            }
            ...
            {
                //hide the window
                WinApi.SetWindowPos(Handle, (IntPtr)0, (iScreenWidth - 4 * iWw - iBGA) / 2, iScreenHeight - iWh, 4 * iWw + iBGA, iWh + iBGA, 0x80);
            }
            else if (iStateLayout == 1)
            {
                SaveLayout();
                WinApi.SetWinFullScreen(Handle);
                ylResetLayout();
            ...
            else if (iStateLayout == 2)
            {
                SaveLayout();
                ylResetLayout();
                //WinApi.SetWindowPos(Handle, (IntPtr)0, 9, 1090, 750, 270, 0);
                WinApi.SetWindowPos(Handle, (IntPtr)0, (iScreenWidth - 4 * iWw - iBGA) / 2, iScreenHeight - iWh, 4 * iWw + iBGA, iWh + iBGA, 0x40);
                //LayoutObjects(0, 0);
            }
            else if ((iStateLayout == 3) || (iStateLayout == 4))
            {
                SaveLayout();
                //WinApi.SetWindowPos(Handle, (IntPtr)0, 9, 40, 750, 1320, 0);
                WinApi.SetWinFullScreen(Handle);
                ylResetLayout();
            }


            //local screen message/position generator
            if (this.iOnscreenMsgCt++ > 6)
            {
                this.iOnscreenMsgCt = 0;
                int randomNumber = Random.Next(0, this.strListOnscreenMsg.Count); // = 0,1,2
                this.strOnscreenMsg = this.strListOnscreenMsg[randomNumber];
                this.iOnscreenMsgX = Random.Next(50, this.iScreenWidth / 2 - 150);
                this.iOnscreenMsgY = Random.Next(50, this.iScreenHeight / 2 - 150);
                this.iOnscreenMsgStyle = Random.Next(1, 3); // = 1,2
