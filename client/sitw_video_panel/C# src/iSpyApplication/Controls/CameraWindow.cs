...
namespace iSpyApplication.Controls
{
    ...
    public sealed class CameraWindow : PictureBox
    {
        ...
        public bool bLocalCam;                      //*YL*
        public bool bInteraction;                   //*YL*
        private int iCtFlashColor = 0;              //*YL* 
        private int iCtFlashAlive = 0;              //*YL* 
        public string strWindowName = "";           //*YL*
        public int iLayout = 2;                     //*YL*

        public string strOnscreenMsg = "";          //*YL*
        public int iOnscreenMsgX = 0;               //*YL*
        public int iOnscreenMsgY = 0;               //*YL*
        public int iOnscreenMsgStyle = 0;           //*YL*
        public int iOnscreenMsgXx = 0;              //*YL*
        public int iOnscreenMsgYy = 0;              //*YL*
        private Random Random = new Random();       //*YL*
        ...
        public CameraWindow(objectsCamera cam)
        {
            ...
            //*YL* -----------------------------------------------------
            if ((Camobject.name.IndexOf("NAE", StringComparison.OrdinalIgnoreCase) >= 0) || (Camobject.name.IndexOf("Art", StringComparison.OrdinalIgnoreCase) >= 0))
            {
                strWindowName = "NAE";
            }
            else if (Camobject.name.IndexOf("Broadway", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                strWindowName = "Broadway";
            }
            else if (Camobject.name.IndexOf("Leytonstone", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                strWindowName = "Leytonstone";
            }
            else if (Camobject.name.IndexOf("Mill", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                strWindowName = "Mill";
            }
            //*YL* -----------------------------------------------------
        ...
        {
            //*YL*----------------------------------
            //var ylDrawfont48 = new Font(FontFamily.GenericSansSerif, 48, FontStyle.Regular, GraphicsUnit.Pixel);
            var ylDrawfont60 = new Font(FontFamily.GenericSansSerif, 60, FontStyle.Regular, GraphicsUnit.Pixel);
            var ylDrawfont90 = new Font(FontFamily.GenericSansSerif, 90, FontStyle.Regular, GraphicsUnit.Pixel);
            var yldrawBrushRed = new SolidBrush(Color.Red);
            var yldrawBrushOrange = new SolidBrush(Color.Orange);
            var yldrawBrushDeepSkyBlue = new SolidBrush(Color.DeepSkyBlue);
            //*YL*----------------------------------
            ...
            Color border = (!Focused) ? Color.Black : Color.Black;
            var grabBrush = new SolidBrush(border);
            //var borderPen = new Pen(grabBrush);
            var borderPen = new Pen(grabBrush, 2);
            //*YL* ----------------------------------------
            ...
            try
            {
                ...
                if (this.bLocalCam)
                {
                    border = Color.Black; //nothing
                    grabBrush = new SolidBrush(border);
                    borderPen = new Pen(grabBrush, 5);
                }

                if ((this.bInteraction) || (iCtFlashAlive > 0))
                {
                    if (this.bInteraction) iCtFlashAlive = 30;
                    ...
                    iCtFlashColor ++;
                    if (iCtFlashColor > 3) iCtFlashColor = 0;
                ...
                //gCam.DrawRectangle(borderPen, 0, 0, rc.Width - 1, rc.Height - 1 );    //*YL*
                gCam.DrawRectangle(borderPen, 2, 2, rc.Width - 4, rc.Height - 4);       //*YL*
                ...
                //int textpos = rc.Height - 15;     //*YL*
                int textpos = rc.Height - 20;       //*YL*
                int yltextpos_y = rc.Height / 2;    //*YL*
                int yltextpos_x = 10;               //*YL*
                ...
                if (Camobject.settings.active)
                {
                    if (_camera != null && !_camera.LastFrameNull)
                    {
                        ...
                        //gCam.DrawImage(bm, rc.X + 1, rc.Y + 1, rc.Width - 2, rc.Height - 26); //*YL*
                        gCam.DrawImage(bm, rc.X + 5, rc.Y + 5, rc.Width - 10, rc.Height - 31); //*YL* - draw video image
                ...
                if (!message)
                {
                    gCam.DrawString(m + Camobject.name, MainForm.Drawfont, drawBrush, new PointF(5, textpos));

                    //*YL*---------------------------------------
                    //gCam.DrawString("This is Nottingham", ylDrawfont, drawBrush, new PointF(15, yltextpos));
                    //gCam.DrawString("New Art Exchange", ylDrawfont, yldrawBrush, new PointF(55, yltextpos + 50)); 

                    if ((this.bLocalCam == true) && (this.iLayout == 1)) 
                    {
                        if (this.iOnscreenMsgStyle == 1)
                        {
                            this.iOnscreenMsgXx = Random.Next(-5, 6);
                            this.iOnscreenMsgYy = Random.Next(-5, 6);
                            gCam.DrawString(this.strOnscreenMsg, ylDrawfont60, yldrawBrushOrange, new PointF(this.iOnscreenMsgX + this.iOnscreenMsgXx, this.iOnscreenMsgY + this.iOnscreenMsgYy));
                        }
                        else if (this.iOnscreenMsgStyle == 2)
                        {
                            this.iOnscreenMsgX += this.iOnscreenMsgXx;
                            this.iOnscreenMsgY += this.iOnscreenMsgYy;
                            gCam.DrawString(this.strOnscreenMsg, ylDrawfont60, yldrawBrushDeepSkyBlue, new PointF(this.iOnscreenMsgX, this.iOnscreenMsgY));
                        }
                        
                    }


                    if ((this.bLocalCam == true) && (this.iLayout == 4))
                    {
                        if (string.Compare(this.strWindowName, "NAE", true) == 0)
                        {
                            gCam.DrawString("Cover central", ylDrawfont90, yldrawBrushRed, new PointF(50, yltextpos_y + 80));
                            gCam.DrawString("button to begin...", ylDrawfont90, yldrawBrushRed, new PointF(10, yltextpos_y + 160));
                        }
                        else
                        {
                            gCam.DrawString("Touch to begin...", ylDrawfont90, yldrawBrushRed, new PointF(yltextpos_x, yltextpos_y + 160));
                        }
                    }
